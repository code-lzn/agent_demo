...
public class DecisionEngine {
     ...
     * <p>
     * 纯响应式：一个 ReAct 轮次 + 完成信号，不做反思、不重试、不循环。
     */
    private Flux<ChatEvent> executeSimple(String userMessage, String conversationId) {
        log.info("[DecisionEngine] 快速路径: 简单对话，纯流式执行");
        return reActExecutor.executeRound(userMessage, conversationId, 1, true)
                .timeout(ROUND_TIMEOUT)
                .concatWith(Flux.just(ChatEvent.done(conversationId, UUID.randomUUID().toString())));
    }
